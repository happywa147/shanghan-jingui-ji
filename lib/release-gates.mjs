...
export function goldenReleaseErrors(goldenData, registryData) {
  ...
  for (const candidate of goldenData.candidates.filter((item) => item.golden_status === "golden")) {
    try {
      ...
      if (candidate.reviewed_relation_type !== current.reviewed_relation_type) errors.push(`${candidate.alignment_id} 人审关系类型与重验结果不一致`);
    } catch (error) {
      errors.push(`${candidate.alignment_id}: ${error.message}`);
    }
  }
  const minimumGolden = registryData.release_policy?.minimum_golden ?? 0;
  const goldenCount = goldenData.candidates.filter((item) => item.golden_status === "golden").length;
  if (goldenCount < minimumGolden) errors.push(`黄金样本数量 ${goldenCount} 低于发布要求 ${minimumGolden}`);
  return errors;
}
