...
const golden = await readJson("data/review/golden-candidates.json");
...
console.log(`同名方比较: ${formulas.comparisons.length}；有差异候选: ${formulas.comparisons.filter((item) => item.has_difference).length}；非句读差异: ${formulas.comparisons.filter((item) => item.substantive_difference).length}`);
console.log(`黄金样本候选: ${golden.candidates.length}；已晋级: ${golden.candidates.filter((item) => item.golden_status === "golden").length}`);
