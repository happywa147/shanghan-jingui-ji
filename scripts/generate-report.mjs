...
<nav><input id="query" placeholder="检索条文编号或原文"><select id="edition"><option value="">全部版本</option><option value="guilin">桂林古本</option><option value="kangping">康平本</option></select><select id="review-filter"><option value="">全部审核状态</option><option value="unreviewed">未审核</option><option value="confirmed">确认对应</option><option value="rejected">否决对应</option><option value="needs_work">待进一步校勘</option></select><button id="export">导出审核</button></nav>
<div id="count"></div>${rows}</main><script>
const key='shanghan-jingui-ji:alignment-reviews:v1',articles=[...document.querySelectorAll('article')],query=document.querySelector('#query'),edition=document.querySelector('#edition'),reviewFilter=document.querySelector('#review-filter'),count=document.querySelector('#count');let reviews=JSON.parse(localStorage.getItem(key)||'{}');
...
function filter(){const q=query.value.trim().toLowerCase(),e=edition.value,r=reviewFilter.value;let shown=0;for(const item of articles){const visible=(!e||item.dataset.edition===e)&&(!r||item.dataset.review===r)&&(!q||item.dataset.search.toLowerCase().includes(q));item.hidden=!visible;if(visible)shown++}count.textContent='显示 '+shown+' / '+articles.length+' 条对照'}
