...
<p class="privacy">审核草稿仅保存在当前浏览器90天，不代表身份验证或服务端批准。共享设备请在导出后清除。</p><noscript><p class="warning">JavaScript 已停用：正文与静态分页仍可阅读；筛选、草稿保存和导出不可用。</p></noscript>${pagination(page, pages)}
...
${rows.join("\n")}${pagination(page, pages)}</main><script src="report.js" defer></script></body></html>`;
}

await mkdir(outputDir, { recursive: true });
for (const file of await readdir(outputDir)) if (new RegExp(`^${stem}-\\d+\\.html$`).test(file)) await unlink(resolve(outputDir, file));
const pages = Math.ceil(variantData.variants.length / pageSize);
for (let page = 1; page <= pages; page++) {
  const rows = variantData.variants.slice((page - 1) * pageSize, page * pageSize).map(row);
  await writeFile(resolve(outputDir, pageName(page)), documentHtml(rows, page, pages));
}
