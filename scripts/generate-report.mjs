...
function save(){try{localStorage.setItem(key,JSON.stringify(reviews))}catch{count.textContent='无法写入浏览器本地存储；请立即导出已有审核。'}}function apply(item){const review=reviews[item.dataset.id]||{status:'unreviewed',note:''};item.querySelector('.status').value=review.status;item.querySelector('.review-note').value=review.note;item.dataset.review=review.status}
...
document.querySelector('#export').addEventListener('click',()=>{const reviewerId=document.querySelector('#reviewer').value.trim();if(!reviewerId){alert('请先填写审核者标识');return}const payload={schema_version:2,exported_at:new Date().toISOString(),reviewer:{id:reviewerId,role:document.querySelector('#role').value,identity_verified:false},input_revision:'${escapeHtml(data.manifest.source_sha256)}',reviews};const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='伤寒金匮集-对照审核.json';link.click();URL.revokeObjectURL(link.href)});
document.querySelector('#next-unreviewed').addEventListener('click',()=>{const visible=articles.filter(item=>!item.hidden&&item.dataset.review==='unreviewed'),current=visible.findIndex(item=>item.contains(document.activeElement)||item.getBoundingClientRect().top>=0),next=visible[(current+1)%visible.length];if(next){next.scrollIntoView({behavior:'smooth',block:'start'});next.querySelector('.status').focus()}});
